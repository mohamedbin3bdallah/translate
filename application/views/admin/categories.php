                  ...
                  <div class="x_content">
					...
					<table id="datatable-buttons" class="table table-striped table-bordered">
					  ...
					  <?php foreach($categories as $key => $category) { ?>
                        <tr>						
						  <td align="center"><?php echo $key; ?></td>						  						  <td align="center">						  <?php if(!empty($category['items'])) { ?>						  <a class="" href="#" data-toggle="modal" data-target="#open-<?php echo $category['id']; ?>"><?php echo lang('details'); ?></a>						   <div id="open-<?php echo $category['id']; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">								<div class="modal-dialog modal-lg">									<div class="modal-content" style="border-top-left-radius:50px; border-bottom-right-radius:50px; background-color:#143E66; color:#fff;">										<div class="modal-header">											<button type="button" class="close" data-dismiss="modal">&times;</button>											<br>											<h3><?php echo lang('langs'); ?></h3>        								</div>										<div class="modal-body">											<table id="datatable-buttons" class="table"  dir="<?php echo $this->loginuser->dir; ?>" style="background-color:#143E66; color:#fff;">												<?php if(!empty($category['items'])) { ?>												<thead>													<tr>														<td><?php echo lang('lang'); ?></td>														<td><?php echo lang('title'); ?></td>													</tr>												</thead>												<?php } ?>												<tbody>												<?php foreach($category['items'] as $item) { ?>													<tr>														<td align="center"><?php echo $item['lang']; ?></td>														<td align="center"><?php echo $item['title']; ?></td>													</tr>												<?php } ?>												</tbody>											</table>										</div>									</div>								</div>							</div>						  <?php } ?>						  </td>
						  <td align="center"><?php if(isset($employees[$category['uid']])) echo $employees[$category['uid']]; ?></td>
						  <td align="center"><?php if($category['time'] != '') { echo ArabicTools::arabicDate($system->calendar.' Y-m-d', $category['time']).' , '.date('h:i:s', $category['time']); if(date('H', $category['time']) < 12) echo ' '.lang('am'); else echo ' '.lang('pm'); } ?></td>
						  <td align="center"><input type="checkbox" <?php if($category['active'] == 1) echo 'checked'; ?> disabled></td>
						  <?php if(strpos($this->loginuser->privileges, ',cgedit,') !== false) { ?><td align="center"><a href="<?php echo base_url(); ?>categories/edit/<?php echo $category['id']; ?>" style="color: #2AA3D6;"><i style="color:#2AA3D6;" class="glyphicon glyphicon-edit"></i></a></td><?php } ?>
						  <?php if(strpos($this->loginuser->privileges, ',cgdelete,') !== false) { ?><td align="center">
							<i id="<?php echo $category['id']; ?>" style="color:red;" class="del glyphicon glyphicon-remove-circle"></i>
							<div id="del-<?php echo $category['id']; ?>" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
								<div class="modal-dialog modal-sm">
									<div class="modal-content">
										...
										<div class="modal-body">
										<center>
											<button class="btn btn-danger" id="action_buttom" onclick="location.href = 'categories/del/<?php echo $category['id']; ?>'" data-dismiss="modal"><?php echo lang('yes'); ?></button>
